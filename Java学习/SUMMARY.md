# Summary

* [Introduction](README.md)
* [git的使用](git的使用\README.md)
  * [取消右键的菜单](git的使用\取消右键的菜单.md)
  * [git命令](git的使用\git命令.md)
  * [git标签操作](git的使用\git标签操作.md)
  * [idea gitstash](git的使用\idea gitstash.md)
  * [linux安装git服务器](git的使用\linux安装git服务器\安装.md)
    * [安装新版git](git的使用\linux安装git服务器\安装新版git.md)
  * [安装gitlab](git的使用\安装gitlab\README.md)
    * [CentOS7安装维护Gitlab](安装gitlab\CentOS7安装维护Gitlab.md)
* [基础知识](README.md)
  * [泛型中占位符T和有什么区别](基础知识\泛型中占位符T和有什么区别.md)
* [代码记录](代码记录\README.md)
  * [1.Double运算比较问题](代码记录\1.Double运算比较问题.md)
  * [2.代码优化问题](代码记录\2.代码优化问题.md)
  * [3.判断是否手机微信浏览器打开](代码记录\3.判断是否手机微信浏览器打开.md)
  * [4.label绑定input触发事件](代码记录\4.label绑定input触发事件.md)
  * [5.jq获取checkedbox选中的值](代码记录\5.jq获取checkedbox选中的值.md)
  * [6.获取主机ip](代码记录\6.获取主机ip.md)
  * [7.校验工具类](代码记录\7.校验工具类.md)
  * [8.jq 定时器](代码记录\8.jq 定时器.md)
  * [9.毫秒转换为秒](代码记录\9.毫秒转换为秒.md)
  * [10.字符串写成流](代码记录\10.字符串写成流.md)
  * [11.File路径新建文件夹创建](代码记录\11.File路径新建文件夹创建.md)
* [JVM](JVM\README.md)
  * [接口响应慢排查](JVM\系统缓慢JVM排查.md)
* [JDK8](JDK8\README.md)
  * [Stream](JDK8\Stream流.md)
  * [Collect](JDK8\collect.md)
* [IDEA学习](IDEA学习\README.md)
  * [IDEA远程debug-jar](IDEA学习\IDEA远程debug-jar.md)
  * [发布应用到 tomcat](IDEA学习\发布应用到 tomcat.md)
  * [IDEA中使用 Debug](IDEA学习\IDEA中使用 Debug .md) 
  * [IDEA好用插件](IDEA学习\IDEA好用插件.md)

